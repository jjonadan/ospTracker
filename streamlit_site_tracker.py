...
st.title("📍 OSP Site Tracker (Streamlit Ver.)")

# Session state for storing remarks temporarily
...
if uploaded_file:
    ...
    # Display site list with remarks button
    for idx, row in filtered_df.iterrows():
        with st.expander(f"📌 {row['Site Name']} ({row['Region']})"):
            ...
            new_remark = st.text_area(f"Write a new remark for {row['Site Name']}", key=f"remark_input_{site_key}")
